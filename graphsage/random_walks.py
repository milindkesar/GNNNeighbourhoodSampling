...
class Cluster_RandomWalk:
    ...
    def __init__(self, graph, walk_length=80, num_walks=10, p=1, q=1, weight_key='weight',
                 workers=1, sampling_strategy=None, quiet=False, temp_folder=None, cluster_labels=None, cluster_weight=0.1, node_features=None):
        """
        ...
        if self.features_exist:
            ...
            if self.features_exist:
                ...
                for item_id in range(self.node_features.shape[0]):
                    ...
                    nearest_neighbors = get_nearest_neighbors(self.node_features, query_vector.reshape(1, -1), model, max_search_radius=2)
                    if len(nearest_neighbors) < 20:
                        radius = 3
                        while True:
                            nearest_neighbors = get_nearest_neighbors(self.node_features, query_vector.reshape(1, -1), model, max_search_radius=radius)
                            if len(nearest_neighbors) > 20:
                                break
                            radius = radius + 1
        ...
        for source in nodes_generator:
            ...
            for current_node in self.graph.neighbors(source):
                ...
                for destination in self.graph.neighbors(current_node):
                    ...
                    if self.graph[current_node][destination].get(self.weight_key,-1) == self.cluster_weight:##Cluster probability
