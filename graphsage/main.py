...
parser.add_argument('--search_radius',default=2,type=float)
parser.add_argument('--n_lsh_neighbours_sample', default=5, type=float)
parser.add_argument('--num_lsh_neigbours',default=10,type=float)
parser.add_argument('--n_vectors',default=16,type=float)
...
if __name__ == "__main__":
    ...
    #print(args.typewalk)
    print(args.p,args.q)
    run_general(name=args.dataset,outdir=args.outdir,rw=rw,neighbours1=args.neighbours1,neighbours2=args.neighbours2,aggregator=args.aggregator,epochs=args.epochs,random_iter=args.iter,n_layers=args.n_layers, attention=args.attention,lr=args.lr,includenodefeats=args.includenodefeats,type_of_walk=args.typewalk, p=args.p, q=args.q, num_walks=args.num_walks, walk_length=args.walk_length, teleport=args.teleport, teleport_khop=teleport_khop, dfactor=args.dfactor, save_predictions=save_predictions, augment_khop=augment_khop,atleast=atleast,n_vectors=args.n_vectors, search_radius=args.search_radius, n_lsh_neighbours_sample=args.n_lsh_neighbours_sample, num_lsh_neigbours=args.num_lsh_neigbours)